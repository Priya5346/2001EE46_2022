...
import os
os.chdir(r'C:\Users\PRIYA RAJ\OneDrive\Documents\GitHub\2001EE46_2022\proj3')
...
st.set_page_config(page_title="CS384 Project 3",page_icon=":fire:",layout="wide")
...
#--Header section
with st.container():
    st.title("Project 3 Web Interface")
    st.header("Made by- Kanishk Giri and Priya Raj")
    st.subheader("2001EE24 and 2001EE46")
    
#--taking the input from user---
with st.container():
    st.header("Chose Filtering Method From Below")
    original_list=['','C','S','A','C & S','C & A','S & A','C & S & A','all combine']
    result=st.selectbox("Chose Filtering Method",original_list)
    for i in range(1,9):
        if(result==original_list[i]):
            if(i==1):
                tch=1
                st.subheader("Enter Threshold value C:")
                print(corr)
                corr=st.text_input("Enter value C",placeholder="please enter a integer C value")
                try:
                    corr=int(corr)
                except Exception as e:
                    st.write("Please enter an integer value for C")
                    print(e)
            elif(i==2):
                tch=2
                st.subheader("Enter thresold value S:")
                SNR=st.text_input("Enter value S",placeholder="please enter a integer S value")
                try:
                    SNR=int(SNR)
                except Exception as e:
                    st.write("Please enter an integer value for S")
                    print(e)
            elif(i==3):
                tch=3
                st.subheader("Enter Lambda value for A:")
                Lambda=st.text_input("Enter Lambda value for A",placeholder="please enter a float Lambda value")
                try:
                    Lambda=float(Lambda)
                except Exception as e:
                    st.write("please enter a float Lambda value")
                    print(e)
                st.subheader("Enter k value for A:")
                k=st.text_input("Enter k value for A",placeholder="please enter a float k value")
                try:
                    k=float(k)
                except Exception as e:
                    st.write("please enter a float k value")
                    print(e)
            elif(i==4):
                tch=4
                st.subheader("Enter Threshold value C:")
...
with st.container():
    st.write("---")
    st.header("Chose Replacement Method From Below")
    st.write("1. previous point")
    st.write("2. 2*last-2nd_last")
    st.write("3. overall_mean")
    st.write("4. 12_point_strategyt")
    st.write("5. mean of previous 2 point")
    st.write("6. all seqential")
    st.write("7. all parallel")
    sch=st.text_input("Enter the Replacement Method",placeholder="Enter number between 1 and 7")
    try:
        sch=int(sch)
        if sch>7 or sch<=0:
            st.write("Please enter a value from given options")
    except Exception as e:
